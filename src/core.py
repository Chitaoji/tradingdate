...
class TradingCalendar:
    ...
    def recover_calendar(self) -> Self:
        """Return a new calendar generated by `self.id`."""
        return TradingCalendar(self.id, CalendarEngine().get_calendar(self.id))
...
class WeekCalendar(TradingCalendar):
    ...
    def __add__(self, value: int, /) -> Self:
        ...
        if value < 0:
            return self - abs(value)
        cal = self.recover_calendar()
        week_list = sorted({hash(x.week) for x in cal})
        idx = week_list.index(hash(self))
        if idx + value < len(week_list):
            y = week_list[idx + value]
            return cal.get_year(str(y)[:4]).get_month(str(y)[4:])
        raise OutOfCalendarError(
...
class DayCalendar(TradingCalendar):
    ...
    def __add__(self, value: int, /) -> Self:
        cal = self.recover_calendar()
        return (cal.get_nearest_date_after(hash(self)) + value).day
